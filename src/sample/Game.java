...
public abstract class Game extends Application implements GameScreen {
    ...
    private void createScorePanel() {
        ...
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(Color.WHITE));
    ...
    private Color fromFXColor(Color color) {
        ...
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }
